...
public class CategoryResource {
    ...
    @Timed
    public ResponseEntity<List<CategoryDTO>> getAllCategories(@ApiParam Pageable pageable, @RequestParam(required = false) String filter) {
        if ("product-is-null".equals(filter)) {
            log.debug("REST request to get all Categorys where product is null");
            return new ResponseEntity<>(categoryService.findAllWhereProductIsNull(),
                    HttpStatus.OK);
        }
